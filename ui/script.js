...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// PROTECTED ACCESS LAYER — Multi-layer verification system
// Unauthorised modification or bypass is strictly prohibited.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
!function(W,D,_u){
'use strict';
// ── Segment A: Runtime string reconstruction via XOR ──────
// All sensitive strings are XOR-encoded with key 0x42 — no plaintext visible.
var _x=function(a,k){return a.map(function(c){return String.fromCharCode(c^(k||66));}).join('');};
// API endpoint  → https://zaibtts.com/check/check.php
var _ep=_x([42,54,54,50,49,120,109,109,56,35,43,32,54,54,49,108,33,45,47,109,33,42,39,33,41,109,33,42,39,33,41,108,50,42,50]);
// localStorage token ID  → _z4ib_t0ken
var _tk=_x([29,56,118,43,32,29,54,114,41,39,44]);
// WA contact number  → 923705568511
var _wn=_x([123,112,113,117,114,119,119,116,122,119,115,115]);
// Cipher passphrase segments  → 'Z4ibTTS!'
var _ph=_x([24,6,11,32,22,22,35,63]);
// ── Segment B: Encrypted localStorage read/write ──────────
var _hn=function(){return(W.location.hostname||'local').slice(0,8);};
var _ek=function(){return _ph+_hn();};
var _enc=function(v){
    var k=_ek(),r='',i=0;
    for(;i<v.length;i++)r+=String.fromCharCode(v.charCodeAt(i)^k.charCodeAt(i%k.length));
    return btoa(unescape(encodeURIComponent(r)));
};
var _dec=function(v){
    try{
        var k=_ek(),s=decodeURIComponent(escape(atob(v))),r='',i=0;
        for(;i<s.length;i++)r+=String.fromCharCode(s.charCodeAt(i)^k.charCodeAt(i%k.length));
        return r;
    }catch(e){return'';}
};
var _lsGet=function(){try{var v=localStorage.getItem(_tk);return v?_dec(v):'';}catch(e){return'';}};
var _lsSet=function(v){try{localStorage.setItem(_tk,_enc(v));}catch(e){}};
var _lsDel=function(){try{localStorage.removeItem(_tk);}catch(e){}};
// ── Segment C: State machine — only controlled transitions ─
var _ST={INIT:0,POPUP:1,GATE:2,UNLOCKED:3};
var _st=_ST.INIT;
var _verified=false;
var _verifyTimer=null;
var _guardianObs=null;
// ── Segment D: DOM lock guardian ──────────────────────────
var _guardGate=function(){
    if(_st===_ST.UNLOCKED)return;
    var g=D.getElementById('access-gate');
    if(!g)return;
    if(g.style.display==='none'||g.style.visibility==='hidden'||g.style.opacity==='0'){
        g.style.cssText='display:flex!important;visibility:visible!important;opacity:1!important;z-index:99999!important;';
    }
};
var _startGuardian=function(){
    if(_guardianObs)return;
    var g=D.getElementById('access-gate');
    if(!g||!W.MutationObserver)return;
    _guardianObs=new MutationObserver(function(){_guardGate();});
    _guardianObs.observe(g,{attributes:true,attributeFilter:['style','class']});
    _guardianObs.observe(D.body,{childList:true,subtree:false});
};
var _stopGuardian=function(){
    if(_guardianObs){_guardianObs.disconnect();_guardianObs=null;}
};
// ── Segment D2: Transport-encrypted fetch interceptor ─────
// After unlock, every same-origin API request automatically carries an encrypted key header.
// Browser sends: X-ZK = base64( XOR(raw_key, TRANSPORT_SECRET) )
// Transport secret (XOR'd with 0x42) → 'Zaib2024xTTS'
var _tsec=_x([24,35,43,32,112,114,112,118,58,22,22,17]);
var _encHeader=function(k){
    var r='',i=0;
    for(;i<k.length;i++)r+=String.fromCharCode(k.charCodeAt(i)^_tsec.charCodeAt(i%_tsec.length));
    return btoa(r);
};
var _patchFetch=function(){
    var _orig=W.fetch;
    W.fetch=function(url,opts){
        var urlStr=String(url||'');
        var isSameOrigin=!urlStr.startsWith('http')||urlStr.indexOf(W.location.host)>-1;
        if(isSameOrigin){
            opts=opts||{};
            var h=opts.headers;
            if(h&&typeof h.set==='function'){h.set('X-ZK',_encHeader(_lsGet()));}
            else{opts.headers=Object.assign({},(h||{}),{'X-ZK':_encHeader(_lsGet())});}
        }
        return _orig.call(W,url,opts);
    };
};
// ── Segment E: Anti-debug timing check ───────────────────
var _dbchk=function(){
    var t=Date.now(),threshold=300;
    (function(){}());
    if(Date.now()-t>threshold&&_st!==_ST.UNLOCKED){
        _lsDel();
        W.location.reload();
    }
};
// ── Segment F: Core verify call ──────────────────────────
var _callApi=function(key){
    return new Promise(function(res,rej){
        var u=_ep+'?key='+encodeURIComponent(key)+'&_='+Date.now();
        fetch(u,{mode:'cors',cache:'no-store',credentials:'omit'})
        .then(function(r){
            if(!r.ok)rej(new Error(r.status));
            return r.json();
        })
        .then(function(j){
            // Layer: validate response structure deeply
            if(typeof j!=='object'||j===null)return rej(new Error('bad_response'));
            if(typeof j.valid!=='boolean')return rej(new Error('bad_format'));
            res(j);
        })
        .catch(rej);
    });
};
// ── Segment G: Show / hide app ───────────────────────────
var _showGate=function(){
    _st=_ST.GATE;
    var g=D.getElementById('access-gate');
    var m=D.getElementById('main-app');
    if(m)m.style.cssText='display:none!important;';
    if(g)g.style.cssText='display:flex!important;';
    _startGuardian();
};
var _unlockApp=function(){
    _stopGuardian();
    _st=_ST.UNLOCKED;
    _verified=true;
    var g=D.getElementById('access-gate');
    ...
    _patchFetch();
    // Periodic silent re-verification every 9 minutes
    if(_verifyTimer)clearInterval(_verifyTimer);
    _verifyTimer=setInterval(function(){
        var k=_lsGet();
        if(!k){_lsDel();_st=_ST.GATE;W.location.reload();return;}
        _callApi(k).then(function(j){
            if(!j.valid){_lsDel();W.location.reload();}
        }).catch(function(){/* network error - stay unlocked */});
    },540000);
};
// ── Segment H: Auth flow ─────────────────────────────────
var _checkAndProceed=function(){
    var saved=_lsGet();
    if(!saved){_showGate();return;}
    _callApi(saved).then(function(j){
        if(j.valid){_unlockApp();}
        else{_lsDel();_showGate();}
    }).catch(function(){
        // Cannot reach server — unlock to not block user (backend validates anyway)
        _unlockApp();
    });
};
// ── Segment I: DOMContentLoaded wiring ───────────────────
D.addEventListener('DOMContentLoaded',function(){
    // Anti-debug probe (fires once on load)
    _dbchk();
    // 1. WhatsApp Popup
    var waP=D.getElementById('wa-channel-popup');
    var waC=D.getElementById('wa-popup-close');
    var waJ=D.getElementById('wa-join-btn');
    _st=_ST.POPUP;
    if(waP&&waC){
        waC.addEventListener('click',function(){waP.style.display='none';_checkAndProceed();});
        if(waJ)waJ.addEventListener('click',function(){setTimeout(function(){waP.style.display='none';_checkAndProceed();},300);});
    }else{
        _checkAndProceed();
    }
    // 2. Gate tab switcher
    var tabs=D.querySelectorAll('.gate-tab');
    tabs.forEach(function(tab){
        tab.addEventListener('click',function(){
            tabs.forEach(function(t){t.classList.remove('active');});
            tab.classList.add('active');
            D.querySelectorAll('.gate-panel').forEach(function(p){p.style.display='none';});
            var tgt=D.getElementById('tab-'+tab.dataset.tab);
            if(tgt)tgt.style.display='block';
        });
    });
    // 3. Key verification
    var vBtn=D.getElementById('verify-key-btn');
    var kIn=D.getElementById('access-key-input');
    var gErr=D.getElementById('gate-error');
    var gOk=D.getElementById('gate-success');
    var _msg=function(el,m){if(el){el.textContent=m;el.style.display='block';}};
    if(vBtn&&kIn){
        vBtn.addEventListener('click',function(){
            var key=(kIn.value||'').trim().toUpperCase();
            if(!key){_msg(gErr,'❌ Please enter your access key.');return;}
            vBtn.disabled=true;
            vBtn.innerHTML='<span>⏳ Verifying...</span>';
            if(gErr)gErr.style.display='none';
            if(gOk)gOk.style.display='none';
            _callApi(key).then(function(j){
                if(j.valid===true&&j.valid!==undefined){
                    _lsSet(key);
                    _msg(gOk,'✅ Access granted! Welcome, '+(j.name||'User')+'!');
                    setTimeout(function(){_unlockApp();},900);
                }else{
                    _msg(gErr,'❌ '+(j.error||'Invalid key. Contact support on WhatsApp.'));
                    vBtn.disabled=false;
                    vBtn.innerHTML='<span>✅ Verify &amp; Activate</span>';
                }
            }).catch(function(){
                _msg(gErr,'⚠️ Could not reach verification server. Check your connection.');
                vBtn.disabled=false;
                vBtn.innerHTML='<span>✅ Verify &amp; Activate</span>';
            });
        });
        kIn.addEventListener('keydown',function(e){if(e.key==='Enter')vBtn.click();});
    }
    // 4. Pay method toggle (PK / International)
    var _activeMethod='pk';
    var _pmBtns=D.querySelectorAll('.pay-method-btn');
    var _secPk=D.getElementById('pay-section-pk');
    var _secIntl=D.getElementById('pay-section-intl');
    _pmBtns.forEach(function(b){
        b.addEventListener('click',function(){
            _pmBtns.forEach(function(x){x.classList.remove('active');});
            b.classList.add('active');
            _activeMethod=b.dataset.method;
            if(_activeMethod==='pk'){
                if(_secPk)_secPk.style.display='';
                if(_secIntl)_secIntl.style.display='none';
            }else{
                if(_secPk)_secPk.style.display='none';
                if(_secIntl)_secIntl.style.display='';
            }
        });
    });

    // 5. Copy pill buttons (phone numbers / IBAN)
    D.querySelectorAll('.copy-pill').forEach(function(btn){
    ...
    // 6. Crypto selector + address display
    var _cryptoAddrs={
        'usdt-trc20':{'addr':'TMuiipRzVV2XpDMzJFTxEDDgJw9rZ6tAa2','net':'USDT TRC20 — TRON Network','warn':'⚠️ Send ONLY on TRC20 (TRON) network. Wrong network = permanent loss.'},
        'usdt-erc20':{'addr':'Contact Zaib on WhatsApp for ERC20 address','net':'USDT ERC20 — Ethereum Network','warn':'⚠️ ERC20 address provided on request via WhatsApp.'},
        'btc':{'addr':'Contact Zaib on WhatsApp for BTC address','net':'Bitcoin (BTC) — Bitcoin Network','warn':'⚠️ Bitcoin address provided on request via WhatsApp.'},
        'eth':{'addr':'Contact Zaib on WhatsApp for ETH address','net':'Ethereum (ETH) — Ethereum Network','warn':'⚠️ ETH address provided on request via WhatsApp.'}
    };
    var _addrEl=D.getElementById('crypto-addr-display');
    var _netEl=D.getElementById('crypto-network-label');
    var _warnEl=D.getElementById('crypto-warn');
    D.querySelectorAll('.crypto-pill').forEach(function(p){
        p.addEventListener('click',function(){
            D.querySelectorAll('.crypto-pill').forEach(function(x){x.classList.remove('active');});
            p.classList.add('active');
            var info=_cryptoAddrs[p.dataset.crypto]||_cryptoAddrs['usdt-trc20'];
            if(_addrEl)_addrEl.textContent=info.addr;
            if(_netEl)_netEl.textContent=info.net;
            if(_warnEl)_warnEl.textContent=info.warn;
        });
    });

    // 7. Copy crypto address button
    var _copyAddrBtn=D.getElementById('copy-crypto-addr');
    var _copyAddrLbl=D.getElementById('copy-addr-label');
    if(_copyAddrBtn){
        _copyAddrBtn.addEventListener('click',function(){
            var addr=(_addrEl&&_addrEl.textContent)||'';
            if(!addr)return;
            navigator.clipboard&&navigator.clipboard.writeText(addr).catch(function(){});
            _copyAddrBtn.classList.add('copied');
            if(_copyAddrLbl)_copyAddrLbl.textContent='✓ Copied!';
            setTimeout(function(){
                _copyAddrBtn.classList.remove('copied');
                if(_copyAddrLbl)_copyAddrLbl.textContent='Copy Address';
            },1800);
        });
    }

    // 8. Pakistani payment → WhatsApp
    var pBtn=D.getElementById('verify-payment-btn');
    var txInPk=D.getElementById('trx-id-pk');
    if(pBtn&&txInPk){
        pBtn.addEventListener('click',function(){
            var trx=(txInPk.value||'').trim();
            if(!trx){txInPk.style.borderColor='#dc2626';txInPk.focus();return;}
            txInPk.style.borderColor='';
            var msg='Hi Zaib! I paid for Unlimited Voice Cloning by Zaib access (Pakistani).\n\nTransaction ID: '+trx+'\n\nPlease verify and send my access key. Thank you!';
            W.open('https://wa.me/'+_wn+'?text='+encodeURIComponent(msg),'_blank');
        });
    }

    // 9. Crypto payment → WhatsApp
    var pBtnIntl=D.getElementById('verify-payment-intl-btn');
    var txInIntl=D.getElementById('trx-hash-intl');
    if(pBtnIntl&&txInIntl){
        pBtnIntl.addEventListener('click',function(){
            var hash=(txInIntl.value||'').trim();
            if(!hash){txInIntl.style.borderColor='#dc2626';txInIntl.focus();return;}
            txInIntl.style.borderColor='';
            var crypto=(D.querySelector('.crypto-pill.active')||{}).textContent||'Crypto';
            var addr=(_addrEl&&_addrEl.textContent)||'';
            var msg='Hi Zaib! I paid for Unlimited Voice Cloning by Zaib access ('+crypto+').\n\nTX Hash: '+hash+'\nWallet: '+addr+'\n\nPlease verify and send my access key. Thank you!';
            W.open('https://wa.me/'+_wn+'?text='+encodeURIComponent(msg),'_blank');
        });
    }
});
